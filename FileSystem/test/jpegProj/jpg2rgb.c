...
static int g_fd;   //显存的文件句柄

static struct fb_var_screeninfo g_tFBVar; //显存的可变参数
static struct fb_fix_screeninfo g_tFBFix; //显存的固定参数
static unsigned char *g_pucFBMem;         //显存的起始地址
static unsigned int g_dwScreenSize;   	  //显存的大小 单位：（字节Byte）

static unsigned int g_dwLineWidth;		  //显存宽度单位：（字节）
static unsigned int g_dwPixelWidth;		  //显存的像素宽度：（字节）
...
static int FBCleanScreen(unsigned int dwBackColor)
{
	...
	switch (g_tFBVar.bits_per_pixel	) //按像素格式分类 8 16 32
	{
	case 8:
	{
...
static int FBShowLine(int iXStart, int iXEnd, int iY, unsigned char *pucRGBArray)
{
	...
	for (iX = iXStart; iX < iXEnd; iX++)
	{
		...
		i += 3;
		FBShowPixel(iX, iY, dwColor); //ARGB   A=0
	}
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	// 设置解压参数,比如放大、缩小 /* 解压因子 scale_num/scale_denom */
//
//	cinfo.scale_num=1;
//	cinfo.scale_denom=2;
