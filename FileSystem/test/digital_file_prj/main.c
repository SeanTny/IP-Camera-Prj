...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		if(0==GetInputEvevt(&ptInputEvent))
		{
			if (ptInputEvent.iVal == INPUT_VALUE_DOWN)
			{
				ShowNextPage();
			}
			else if (ptInputEvent.iVal == INPUT_VALUE_UP)
			{
				ShowPrePage();
			}
			else if (ptInputEvent.iVal == INPUT_VALUE_EXIT)
			{
				return 0;
			}
		}
